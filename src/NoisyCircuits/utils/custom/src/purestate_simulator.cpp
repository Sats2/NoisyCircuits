...
static inline void apply_single_qubit_noise(complex128* __restrict__ state, const std::size_t q, const std::size_t q_null, const std::size_t num_qubits, const std::list<matrix>& noise_operators){
    ...
    for (const auto& oper : noise_operators){
...
static inline void apply_two_qubit_noise(complex128* __restrict__ state, const std::size_t q1, const std::size_t q2, const std::size_t num_qubits, const std::list<matrix>& noise_operators){
    const int num_operators = noise_operators.size();
    const std::size_t dim = std::size_t{1} << num_qubits;
...
void pure_state(const std::list<ItemEntry> instruction_list, const std::size_t num_qubits, complex128* __restrict__ state){
    std::map<std::string, void(*)(complex128* __restrict__, const std::size_t, const std::size_t, const std::size_t, const double)> gate_map;
...
// Apply parallel trajectory computation here!
void mcwf_state(const std::list<ItemEntry> instruction_list, const std::vector<noise_map>& single_qubit_instructions, noise_map2q two_qubit_instructions, const std::size_t num_qubits, complex128* __restrict__ state){
    std::map<std::string, void(*)(complex128* __restrict__, const std::size_t, const std::size_t, const std::size_t, const double)> gate_map;
    ...
    std::map<std::string, void(*)(complex128* __restrict__, const std::size_t, const std::size_t, const std::size_t, const std::list<matrix>&)> noise_map;
    noise_map["x"] = apply_single_qubit_noise;
    noise_map["rz"] = apply_single_qubit_noise;
    noise_map["rx"] = apply_single_qubit_noise;
    noise_map["sx"] = apply_single_qubit_noise;
    noise_map["cz"] = apply_two_qubit_noise;
    noise_map["rzz"] = apply_two_qubit_noise;
    noise_map["ecr"] = apply_two_qubit_noise;
...
void simulate_circuit(py::list instructions, py::array_t<complex128> statevector, py::dict single_qubit_noise_instructions, py::dict two_qubit_noise_instructions, std::size_t num_qubits, bool noisy, uint8 thread_count){
    ...
    if (noisy){
        ...
        mcwf_state(instruction_list, single_qubit_instructions, two_qubit_instructions, num_qubits, state);
    }
    else{
        uint8 max_use_threads = std::max<uint8>(thread_count, num_qubits / 4);
...
PYBIND11_MODULE(simulator, m){
    ...
    m.def("simulate_circuit", &simulate_circuit, "Simulates a quantum circuit with and without noise", py::arg("instructions"), py::arg("statevector"), py::arg("noise_instructions"), py::arg("num_qubits"), py::arg("noisy"), py::arg("thread_count"));
}