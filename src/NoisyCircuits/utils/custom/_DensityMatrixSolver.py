...
Example:
    >>> import numpy as np
    >>> from NoisyCircuits.utils.custom import DensityMatrixSolver
    >>> instruction_list = []
...
class DensityMatrixSolver:
    ...
    def solve(self,
        ...
        -------
        np.ndarray[np.float64]
            Probabilities of measuring the specified qubits in the computational basis.
        """
        ...
        instruction_map = {
            ...
            "unitary" : lambda p, q: circuit.unitary(p, q)
        }
        ...
        sim = AerSimulator(noise_model = self.noise, method = "density_matrix")
        ...
        probs = np.diag(np.asarray(partial_trace(res.data()["density_matrix"], trace_qubits))).real
        return np.require(probs, dtype=np.float64, requirements=["C"])