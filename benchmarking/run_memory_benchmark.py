...
def generate_random_circuit(qubits, file_loc, depth=100):
    subprocess.run(["python3", "initialization_script.py", str(qubits), str(depth), str(file_loc)])
...
for qubits in range(2, max_qubits + 1):
    ...
    for trial in range(trials):
        ...
        generate_random_circuit(qubits, circuit_loc)
        ...
        for trajectory_number in trajectory_list:
            ...
            qubit_data[trajectory_number].append(memory_usage)
    ...
    data["density_matrix"][qubits] = density_matrix_memory_usages_full

    for trajectory_number in trajectory_list:
        mean_total_usage = np.mean(qubit_data[trajectory_number])
        std_total_usage = np.std(qubit_data[trajectory_number])
        text += "Trajectory Number: {}\n".format(trajectory_number)
        text += "Mean Maximum Memory (RSS) Usage: {:.2f} MB, Std Dev: {:.2f} MB\n".format(mean_total_usage, std_total_usage)
...
def plot_data(data, file_loc, traj_list=trajectory_list):
    ...
    for qubits in qubit_list:
        mean_mem_usage_dm.append(np.mean(data["density_matrix"][qubits]))
        std_mem_usage_dm.append(np.std(data["density_matrix"][qubits]))
    ...
    for traj in traj_list:
        ...
        std_mem_usage_mcwf = []
        for qubits in qubit_list:
            mean_mem_usage_mcwf.append(np.mean(data["mcwf"][qubits][traj]))
            std_mem_usage_mcwf.append(np.std(data["mcwf"][qubits][traj]))
    ...
    plt.ylabel("Maximum Memory Usage (RSS) [MB]")
    plt.title("Peak RSS Memory Usage of Density Matrix vs MCWF Simulations (Process Memory Usage)")
    plt.legend()
    plt.grid()
